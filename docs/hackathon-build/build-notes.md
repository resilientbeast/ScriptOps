...
## 2026-09-05 — Build item 9: evidence-aware ADK Revision Ripple

- Replaced the lifecycle bridge with an official ADK 2.0 Revision Ripple crew: Breakdown, Parallel Evidence, Schedule, and parallel Budget/Locations/Casting workers use distinct output keys, bounded provider timeouts, strict schema gates, token-owned Firestore stage writes, and a final proposal contract.
- Added normalized Parallel evidence persistence with attribution and a controlled cache/fallback resolver. The activity rail now exposes provider provenance, evidence count, cited sources, and each of the five affected artifact sections without implying that the unapproved proposal changed the baseline.
- Deployed golden run `131c1ea1-ea3e-5822-a84f-ad7b6126e67b` completed all six stages with eight live, cited Parallel evidence records and the complete Scene 14 production consequences.
- Resilience verification deliberately deployed revision `scriptops-00020-8bw` with a 1 ms Parallel timeout. Run `8f57fe49-20d4-57f2-b912-91545abe7c0c` still completed all six stages through the disclosed cached-evidence path, retained eight cited records and all five impact sections, and left the browser-scoped baseline at Plan v1 with no approved run. Normal 30-second behavior was restored in serving revision `scriptops-00021-wqk`.
