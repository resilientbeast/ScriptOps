...
function approvedPlans(sceneCount = 6) {
  ...
  const rippleDraft = createProjectRippleDraft({ jobId: "ripple-export", planningInputs: fixture.snapshot.planningInputs, base: v1, sceneId: fixture.plan.scenes[0]!.id, requestText: "Move the opening scene to a covered weather day and revise the schedule.", evidence: fixture.evidence, output: { schedule: fixture.plan.schedule, assumptions: ["Covered day access needs producer confirmation."], warnings: ["Production estimates need producer review."] } });
