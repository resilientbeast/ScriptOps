...
export function planningFixture(count = 1, people = false) {
  ...
  const budgetItems = [["crew-day", "Crew payroll", 4500, 6000], ["cast", "Cast performers", 1000, 1500], ["equipment", "Camera and lighting equipment", 1500, 2000], ["locations", "Locations and permits", 1000, 1250], ["transport", "Transport and fuel", 750, 1000], ["catering", "Catering and craft services", 750, 1000], ["insurance", "Insurance and production admin", 1000, 1250], ["post", "Post-production", 2500, 3000], ["contingency", "Contingency and weather cover", 1500, 2000]] as const;
  const budget = { currency: "USD", low: 14500, high: 19000, lineItems: budgetItems.map(([id, category, low, high]) => ({ id, category, quantity: 1, unit: "project allowance", low, high, basis: "Provisional whole-production allowance requiring producer quotes.", evidenceIds: [evidence[0]!.id] })), costDrivers: [{ id: "crew", label: "Crew and equipment", direction: "neutral" as const, reason: "Small crew and equipment package scaled to the planned shoot days.", evidenceIds: [evidence[0]!.id] }], assumptions: ["A small nonunion crew; rates require producer quotes."] };
  const plan: InitialProductionPlan = { title: snapshot.projectTitle, logline: null, currency: "USD", scenes, schedule: { shootDays: Math.ceil(count / 5), days: Array.from({ length: Math.ceil(count / 5) }, (_, i) => ({ id: `day-${i + 1}`, dayNumber: i + 1, label: `Day ${i + 1}`, sceneIds: scenes.slice(i * 5, (i + 1) * 5).map(scene => scene.id), dayNight: "day", estimatedHours: 8, setupRequirements: ["Allow two hours for setup"], complianceNotes: [] })) }, budget, locations: [{ id: "location-one", name: people ? "Workshop interior" : "Desert exterior", locality: "New Mexico", regionCode: "US-NM", fit: "Matches the screenplay setting", risks: ["Access needs producer verification"], evidenceIds: [evidence[0]!.id] }], casting, evidence: evidence.map(({ id, title, url, retrievedAt }) => ({ id, title, url, retrievedAt })), assumptions: [], warnings: [] };
  const calls: string[] = [];
