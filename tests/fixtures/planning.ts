...
export function planningFixture(count = 1, people = false) {
  ...
  const budgetItems = [["crew-day", "Crew payroll", 1000, 2000], ["equipment", "Camera and lighting equipment", 500, 1000], ["locations", "Locations and permits", 250, 500], ["transport", "Transport and fuel", 250, 500], ["catering", "Catering and craft services", 200, 400], ["insurance", "Insurance and production admin", 250, 500], ["post", "Post-production", 200, 400], ["contingency", "Contingency and weather cover", 350, 700]] as const;
  const budget = { currency: "USD", low: 3000, high: 6000, lineItems: budgetItems.map(([id, category, low, high]) => ({ id, category, quantity: 1, unit: "project allowance", low, high, basis: "Provisional whole-production allowance requiring producer quotes.", evidenceIds: [evidence[0]!.id] })), costDrivers: [{ id: "crew", label: "Crew and equipment", direction: "neutral" as const, reason: "Small crew and equipment package scaled to the planned shoot days.", evidenceIds: [evidence[0]!.id] }], assumptions: ["A small nonunion crew; rates require producer quotes."] };
